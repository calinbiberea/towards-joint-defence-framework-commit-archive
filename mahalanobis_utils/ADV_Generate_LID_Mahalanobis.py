...
parser.add_argument('--dataset', required=True, help='cifar10 | cifar100 | svhn | mnist | fashion')
parser.add_argument('--dataroot', default='./data', help='path to dataset')
parser.add_argument('--outf', default='./adv_output/', help='folder to output results')
parser.add_argument('--num_classes', type=int, default=10, help='the # of classes')
parser.add_argument('--net_type', required=True, help='resnet | densenet | lenet')
parser.add_argument('--gpu', type=int, default=0, help='gpu index')
parser.add_argument('--adv_type', required=True, help='FGSM | BIM | DeepFool | CWL2')
parser.add_argument('--train', required=True, help='true | false')
...
def main():
    ...
    if args.net_type == 'densenet':
        ...
        if args.dataset == 'cifar10':
            model.load_state_dict(torch.load("./pre_trained/cifar10_framework").module.state_dict())
        else:
            model.load_state_dict(torch.load("./pre_trained/svhn_framework").module.state_dict())
        in_transform = transforms.Compose([transforms.ToTensor()])

    elif args.net_type == 'lenet':
        if args.dataset == 'mnist':
            model = torch.load("./pre_trained/mnist_framework")
        else:
            model = torch.load("./pre_trained/fashion_mnist_framework")
    ...
    if args.train == 'true':
        test_clean_data = torch.load(args.outf + 'train_clean_data_%s_%s_%s.pth' % (args.net_type, args.dataset, args.adv_type))
        test_adv_data = torch.load(args.outf + 'train_adv_data_%s_%s_%s.pth' % (args.net_type, args.dataset, args.adv_type))
        test_noisy_data = torch.load(args.outf + 'train_noisy_data_%s_%s_%s.pth' % (args.net_type, args.dataset, args.adv_type))
        test_label = torch.load(args.outf + 'train_label_%s_%s_%s.pth' % (args.net_type, args.dataset, args.adv_type))
    else:
        ...
        test_label = torch.load(args.outf + 'test_label_%s_%s_%s.pth' % (args.net_type, args.dataset, args.adv_type))

    # set information about feature extaction
    model.eval()
    # depends if cifar10 or mnist/fmnist
    ...
    for magnitude in m_list:
        ...
        if args.train == 'true':
            file_name = os.path.join(args.outf, 'train_Mahalanobis_%s_%s_%s.npy' % (str(magnitude), args.dataset, args.adv_type))
        else:
            file_name = os.path.join(args.outf, 'test_Mahalanobis_%s_%s_%s.npy' % (str(magnitude), args.dataset, args.adv_type))
