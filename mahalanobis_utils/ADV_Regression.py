...
def main():
    ...
    if args.net_type == 'lenet':
        dataset_list = ['mnist', 'fashion'] #, 'cifar10' 'cifar100', 'svhn']
    else:
    ...
    print('evaluate the Mahalanobis estimator against CLEAN DATA')
    score_list = ['Mahalanobis_0.0', 'Mahalanobis_0.01', 'Mahalanobis_0.005', \
                  'Mahalanobis_0.002', 'Mahalanobis_0.0014', 'Mahalanobis_0.001', 'Mahalanobis_0.0005']
    ...
    for dataset in dataset_list:
        ...
        for out in adv_test_list:
            for score in score_list:
                ...
                train_X, train_Y = lib_regression.load_characteristics(score, dataset, out, outf, train=True)
                lr = LogisticRegressionCV(n_jobs=-1).fit(train_X, train_Y)

                # Get the test data against all attacks
                for out2 in adv_test_list:
                    test_X, test_Y = lib_regression.load_characteristics(score, dataset, out2, outf, train=False)

                    results = lib_regression.detection_performance(lr, test_X, test_Y, outf)
