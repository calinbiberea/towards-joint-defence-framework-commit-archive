...
def load_characteristics(score, dataset, out, outf, train=True):
    """
    ...
    if train:
        file_name = os.path.join(outf, "train_%s_%s_%s.npy" % (score, dataset, out))
    else:
        file_name = os.path.join(outf, "test_%s_%s_%s.npy" % (score, dataset, out))
